...
class Holidays_model extends Model
{
   ...
   protected $allowedFields = ['title', 'date', 'month'];
   protected $searchFields = ['title', 'date',];
   ...
   public function filter($search = null, $limit = null, $start = null, $orderField = null, $orderDir = null)
   {
      ...
      foreach ($query as $index => $value) {
         ...
         $query[$index]['date'] = dates_format($query[$index]['date']);
         $query[$index]['month'] = get_month($query[$index]['month']);
   ...
   {
      $query = $this->get(); // Equivalent to "SELECT * FROM settings"
      $result = $query->getRow(); // Equivalent to "$query->row()"
      return $result;
   }
}